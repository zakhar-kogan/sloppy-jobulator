...
Now: `B3 + F1 + G1` advanced: trusted Supabase role claims are enforced; moderation now supports state patch, merge, and explicit override flows with provenance audit events; public postings support detail/filter/sort/search/pagination with integration tests.
Next: Harden `G1` search/filter edge semantics and CI safety gates for fast vs DB-backed integration checks; then start `A3/F2` policy/bootstrap automation.
