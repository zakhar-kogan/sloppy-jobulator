...
_ASYNC_PG_INSTRUMENTOR = AsyncPGInstrumentor()
...
def setup_api_telemetry(app: FastAPI, settings: Settings) -> TelemetryRuntime:
    ...
    _FASTAPI_INSTRUMENTOR.instrument_app(app, excluded_urls="/health")
    _ASYNC_PG_INSTRUMENTOR.instrument()
    _HTTPX_INSTRUMENTOR.instrument()

    logger.info("OpenTelemetry initialized for API service=%s", settings.otel_service_name)
    return TelemetryRuntime(enabled=True, provider=provider)


def shutdown_api_telemetry(app: FastAPI, runtime: TelemetryRuntime) -> None:
    if not runtime.enabled:
        return
    _FASTAPI_INSTRUMENTOR.uninstrument_app(app)
    _ASYNC_PG_INSTRUMENTOR.uninstrument()
...
def _build_exporter(settings: Settings) -> OTLPSpanExporter | None:
    ...
    if not endpoint:
        logger.info("OTel exporter endpoint not set; spans remain local-only for service=%s", settings.otel_service_name)
        return None
