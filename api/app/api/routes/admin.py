...
from app.schemas.admin import (
    ...
    URLNormalizationOverrideEnabledPatchRequest,
    URLNormalizationOverrideOut,
    URLNormalizationOverrideUpsertRequest,
)
...
async def list_jobs(
    ...
    kind: AdminJobKindFilter | None = Query(default=None),
...
@router.get("/url-normalization-overrides", response_model=list[URLNormalizationOverrideOut])
async def list_url_normalization_overrides(
    ...
    limit: int = Query(default=50, ge=1, le=200),
    offset: int = Query(default=0, ge=0),
) -> list[URLNormalizationOverrideOut]:
    try:
        principal.require_scopes({"admin:write"})
    except PermissionError as exc:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(exc)) from exc
    ...
    try:
        rows = await repository.list_url_normalization_overrides(
            domain=domain,
            enabled=enabled,
            limit=limit,
            offset=offset,
    ...
    return [URLNormalizationOverrideOut(**row) for row in rows]


@router.put("/url-normalization-overrides/{domain}", response_model=URLNormalizationOverrideOut)
async def put_url_normalization_override(
    domain: str,
    payload: URLNormalizationOverrideUpsertRequest,
    principal=Depends(get_human_principal),
    repository=Depends(get_repository),
) -> URLNormalizationOverrideOut:
    try:
    ...
    try:
        row = await repository.upsert_url_normalization_override(
            domain=domain,
            strip_query_params=payload.strip_query_params,
            strip_query_prefixes=payload.strip_query_prefixes,
            strip_www=payload.strip_www,
    ...
    return URLNormalizationOverrideOut(**row)


@router.patch("/url-normalization-overrides/{domain}", response_model=URLNormalizationOverrideOut)
async def patch_url_normalization_override_enabled(
    domain: str,
    payload: URLNormalizationOverrideEnabledPatchRequest,
    principal=Depends(get_human_principal),
    repository=Depends(get_repository),
) -> URLNormalizationOverrideOut:
    try:
    ...
    try:
        row = await repository.set_url_normalization_override_enabled(
            domain=domain,
            enabled=payload.enabled,
            actor_user_id=principal.actor_id,
        )
