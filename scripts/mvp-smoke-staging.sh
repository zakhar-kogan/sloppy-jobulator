...
external_id="staging-smoke-${stamp}"
...
{"origin_module_id":"${connector_module_id}","external_id":"${external_id}","discovered_at":"${discovered_at}","url":"${canonical_url}","title_hint":"Staging Smoke Position","text_hint":"MVP smoke flow","metadata":{"source":"mvp-smoke"}}
JSON
)"
...
{"status":"done","result_json":{"dedupe_confidence":0.99,"risk_flags":["manual_review_low_confidence"],"posting":{"title":"Staging Smoke Position","organization_name":"Example University","canonical_url":"${canonical_url}","normalized_url":"${canonical_url}","canonical_hash":"${canonical_hash}","country":"US","remote":true,"tags":["smoke","mvp"],"areas":["testing"],"description_text":"MVP smoke posting","source_refs":[{"kind":"discovery","id":"${discovery_id}"}]}}}
JSON
)"
...
match = next((row for row in rows if row.get("canonical_url", "").startswith("https://example.edu/jobs/staging-smoke-")), None)
